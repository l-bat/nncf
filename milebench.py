...
# This logic is largely copied from the https://github.com/MileBench/MileBench/tree/main
...
if __name__ == "__main__":
    ...
    if args.enable_eviction:
        eviction_parameters = KVCacheCompressionParameters(
            ...
            refined_algorithm=args.refined_algorithm,
        )
        compress = KVCacheCompressor(eviction_parameters=eviction_parameters)

    sparse_prefill = SparsePrefill(algorithm=args.prefill_impl)
    ...
    with torch.no_grad():
        ...
        for p_idx, data_sample in tqdm(enumerate(data)):
            ...
            image_embeds = get_inputs_embeds(model, inputs, num_keep_tokens=128)
            kwargs = {}
            if "image_sizes" in inputs:
                kwargs["image_sizes"] = inputs.image_sizes

            with sparse_prefill(model), compress(model) if args.enable_eviction else nullcontext():
                generate_ids = model.generate(
