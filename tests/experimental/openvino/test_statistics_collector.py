...
from tests.experimental.openvino.models import OVMultiResultModel
# from tests.experimental.openvino.models import OVMatMulActModel
...
def test_statistics_aggregator(range_type, test_parameters):
    ...
    #     inp_name, shape = inp.get_any_name(), inp.shape
    #     input_data[inp_name] = np.random.rand(*shape).astype(np.float32)
    # core = ov.Core()
    # compiled_model = core.compile_model(model=model, device_name="CPU")
    # outputs = compiled_model(input_data)
    # for i, _ in enumerate(outputs):
    #     print(outputs[compiled_model.output(i)])
    # print('---------------------------------------------------------------')
    # core = ov.Core()
